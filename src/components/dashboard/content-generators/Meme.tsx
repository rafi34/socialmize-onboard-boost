...
export function Meme({ content }: MemeProps) {
  const [copiedIndex, setCopiedIndex] = useState<number | null>(null);

  if (!content || !content.content || !content.content.captions) {
    return <div className="text-muted-foreground text-center py-4">No meme captions generated yet.</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <h3 className="font-medium">Generated Meme Captions</h3>
          ...
          <div key={index} className="border rounded-md p-3">
            <div className="flex justify-between items-start mb-2">
              <div>
                ...
                {caption.meme_style && (
                  <span className="text-xs text-muted-foreground">Style: {caption.meme_style}</span>
                )}
              </div>
              <Button 
                variant="ghost" 
