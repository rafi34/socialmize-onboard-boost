      ...
      return (
        <div className="prose prose-sm max-w-none dark:prose-invert">
          {readableText.split("\n").map((line, index) => (
            <p 
              key={index} 
              className={`my-1 ${line.trim().startsWith("-") ? "pl-4" : ""} ${line.trim().startsWith("Phase") || line.trim().startsWith("Week") ? "text-lg font-medium text-socialmize-brand-green mt-4" : ""}`}
            >
              {line || "\u00A0"}
            </p>
          ))}
      ...
      <DialogContent className="max-w-3xl h-[85vh] flex flex-col overflow-hidden p-0">
        <DialogHeader className="p-6 pb-2">
          <div className="flex items-center gap-2">
            <Brain className="h-6 w-6 text-socialmize-brand-green" />
            ...
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-l-lg ${
                displayMode === 'readable' 
                  ? 'bg-socialmize-brand-green text-white' 
